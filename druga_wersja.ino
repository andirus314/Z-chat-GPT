...
#include "HX711.h"
#include "hasla.h" // Dodany plik zawierający dane sieciowe
...
void calibrateScale() {
  const int numKnownWeights = 2; // Liczba znanych wag
  float knownWeights[numKnownWeights]; // Znane wagi w kilogramach
  float knownReadings[numKnownWeights]; // Odczyty czujnika odpowiadające znanym wagom

  Serial.println("Rozpoczęcie kalibracji...");
  Serial.println("Podaj znane wagi w kilogramach.");

  // Wprowadzenie znanych wag
  for (int i = 0; i < numKnownWeights; i++) {
    Serial.print("Podaj wagę ");
    Serial.print(i + 1);
    Serial.print(": ");
    while (!Serial.available()) {} // Oczekiwanie na dane z konsoli
    knownWeights[i] = Serial.parseFloat();
    Serial.print("Wprowadzona waga: ");
    Serial.println(knownWeights[i]);
  }

  // Odczytanie wartości czujnika dla znanych wag
  for (int i = 0; i < numKnownWeights; i++) {
    Serial.print("Umieść wagę ");
    Serial.print(knownWeights[i]);
    Serial.println(" kg na czujniku i naciśnij Enter.");
    while (!Serial.available()) {} // Oczekiwanie na wciśnięcie klawisza na konsoli
    Serial.read(); // Odczytanie zbędnego znaku z bufora wejściowego
    knownReadings[i] = scale.get_units();
    Serial.print("Odczyt: ");
    Serial.println(knownReadings[i]);
  }

  // Obliczenie współczynnika skali
  float sumWeights = 0;
  float sumReadings = 0;
  for (int i = 0; i < numKnownWeights; i++) {
    sumWeights += knownWeights[i];
    sumReadings += knownReadings[i];
  }
  scaleFactor = sumWeights / sumReadings;

  Serial.println("Kalibracja zakończona.");
  Serial.print("Współczynnik skali: ");
